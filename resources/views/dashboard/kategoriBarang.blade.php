                ...
                <div class="table-responsive">
                    <table class="table align-middle table-row-dashed fs-6 gy-5"
                        ...
                        <tbody class="fw-semibold text-gray-700">
                            @foreach ($kategoriBarangList as $index => $kategoriBarang)
                            <tr>
                                <td class="text-center">{{ $index + 1 }}</td>
                                <td>{{ $kategoriBarang->nama }}</td>
                                <td>Rp. {{ number_format($kategoriBarang->harga, 0, ',', '.') }}</td>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{route ('kategoriBarang.store') }}" id="formTambahKategori" method="POST" enctype="multipart/form-data">
                    @csrf

                    <!-- Nama Kategori -->
                    <div class="mb-3">
                        <label for="nama" class="form-label">Katgeori Barang</label>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                @if (isset($kategoriBarang) && $kategoriBarang !== null)
                <form action="{{ route('kategoriBarang.update', ['id' => $kategoriBarang->id ?? '']) }}" method="POST" id="editKategoriForm" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')

                    <div class="mb-3">
                        <label for="nama" class="form-label">Kategori Produk</label>
                        <input type="text" class="form-control" name="nama" id="nama" value="{{ old('nama', $kategoriBarang->nama) }}" required>
                    </div>

                    <div class="mb-3">
                        <label for="harga" class="form-label">Harga</label>
                        <input type="text" class="form-control" name="harga" id="harga" value="{{ old('harga', $kategoriBarang->harga) }}" required>
                    </div>
...
<script>
    document.getElementById("formTambahKategori").addEventListener("submit", function(event) {
        ...
        fetch("{{ route('kategoriBarang.store') }}", {
                method: "POST",
                headers: {
            ...
            button.addEventListener("click", function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            url: '/dashboard/kategori-barang/' + kategoriId, // Gantilah dengan route yang benar
                            method: 'DELETE',
                            headers: {
